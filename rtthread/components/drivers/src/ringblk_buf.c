 ...
 *            NULL: get failed
 */
rt_rbb_blk_t rt_rbb_blk_get_all_by_index(rt_rbb_t rbb, rt_size_t index)
{
    rt_base_t level;
    rt_rbb_blk_t block = NULL;
    rt_slist_t *node;
    ...
    for (node = rt_slist_first(&rbb->blk_list); node; node = rt_slist_next(node))
    {
        ...
        if (block->status == RT_RBB_BLK_GET)
        {
            if (count == index)
                goto __exit;
            count++;
...
RTM_EXPORT(rt_rbb_blk_get_all_by_index);

/**
 * get the count of PUT GET status blocks in the ring block buffer
 *
 * @param rbb ring block buffer object
 *
 * @return count of PUT status blocks
 */
rt_size_t rt_rbb_blk_get_all_count(rt_rbb_t rbb)
{
    rt_base_t level;
    rt_size_t count = 0;
    rt_slist_t *node;
    ...
    {
        block = rt_slist_entry(node, struct rt_rbb_blk, list);
        if (block->status == RT_RBB_BLK_PUT || block->status == RT_RBB_BLK_GET)
        {
            count++;
        }
    }
...
RTM_EXPORT(rt_rbb_blk_get_all_count);
#endif
/**
