                            ...
                            rt_uint16_t           length)
{
    ...
    if (size == 0)
    {
        #if RT_RINGBUFFER_DEBUG
        rb->total_puterr_size += length;
        if (rb->max_puterr_size < length)
            rb->max_puterr_size = length;
        #endif
        return 0;
    }
    ...
    {
        #if RT_RINGBUFFER_DEBUG
        rb->total_puterr_size += (length - size);
        if (rb->max_puterr_size < length - size)
            rb->max_puterr_size = length - size;
        #endif
        length = size;
    }
...
#if RT_RINGBUFFER_DEBUG
    if (rb->max_used_size < len)
        rb->max_used_size = len;
#endif
    return len;
}
