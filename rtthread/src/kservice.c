...
/* use float */
#define RT_PRINT_HAS_FLOAT

#ifdef RT_PRINT_HAS_FLOAT
    #include <math.h>
#endif /* RT_PRINT_HAS_FLOAT */
/**
    ...
    *sign = 0;
    ...
    if (fi != 0)
    {
        ...
        while (fi != 0)
        {
            ...
            *--p1 = (int)((fj + .03) * 10) + '0';
            r2++;
            //Avoid overwrite stack if arg is NaN/-INF/+INF on simulator
            RT_ASSERT(r2 < CVTBUFSIZE - 1);
        }
...
__ROM_USED char *ecvtbuf(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
    return cvt(arg, ndigits, decpt, sign, buf, 1);
}

__ROM_USED char *fcvtbuf(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
...
static void parse_float(double value, char *buffer, char fmt, int precision)
{
    ...
    if (fmt == 'e')
    {
        ...
        *buffer++ = *digits;
        ...
        rt_memcpy(buffer, digits + 1, precision);
        buffer += precision;
        *buffer++ = capexp ? 'E' : 'e';
...
#ifdef RT_PRINTF_PRECISION
    static char *flt(char *str, char *end, double num, int size, int precision, char fmt, int flags)
#else
    static char *flt(char *str, char *end, double num, int size, char fmt, int flags)
#endif
{
    char tmp[80];
    char c, sign;
    int n, i;
#ifndef RT_PRINTF_PRECISION
    int precision = -1;
#endif
    ...
    if (precision < 0)
        precision = 2; // Default precision: 2
    ...
    for (i = 0; (i < n) && (str < end); i++) *str++ = tmp[i];
...
__ROM_USED rt_int32_t rt_vsnprintf(char       *buf,
                                   rt_size_t   size,
                                   const char *fmt,
                                   va_list     args)
{
...
#ifdef RT_PRINT_HAS_FLOAT
        case 'f':
            str = flt(str, end, va_arg(args, double), field_width, precision, *fmt, flags | SIGN);
            continue;
#endif /* RT_PRINT_HAS_FLOAT */
